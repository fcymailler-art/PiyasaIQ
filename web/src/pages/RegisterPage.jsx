...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!name || !email || !password) { toast.error('Lütfen tüm alanları doldurun.'); return; }
    ...
    try {
      ...
      toast.error(err.message || 'Kayıt başarısız. Email zaten kullanımda olabilir.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Helmet><title>Kayıt Ol | PiyasaIQ</title></Helmet>
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">Ücretsiz başlayın, kredi kartı gerekmez</p>
          </div>

          <div className="glass-card p-8">
            <form onSubmit={handleRegister} className="space-y-5">
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">Ad Soyad</label>
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-muted-foreground">
              Kayıt olarak <span className="text-primary">Kullanım Koşulları</span>'nı kabul etmiş olursunuz.
            </p>
          </div>
        </motion.div>
      </div>
